...
export class AttestationHistoryService {
  ...
  getNotifications({ page = 1, limit = 25 }, filters = {}) {
    const params = { page, nocache: (new Date()).getTime(), sort: '[("time",-1)]' };
  ...
  getRemediation(id, data, etag) {
    ...
    return this.http.patch(`${api_address}/${id}`, data, { headers: { 'If-Match': etag } });
  }
}

export default AttestationHistoryService;
