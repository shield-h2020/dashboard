...
export class DashboardService {
  ...
  getTotalAttacksByDay(startTime, endTime, period, tenant, attack) {
    ...
    let url = `${INFLUX_ADDRESS}/query?db=cyberattack&q=SELECT COUNT("duration") FROM "attack" WHERE time >= '${startTime}' AND time <= '${endTime}'`;
    if (tenant) {
    ...
    url += `GROUP BY ${period},attack_type`;
    return this.http.get(
      url, {
        headers: {
          Authorization: `Basic ${this.window.btoa(`${this.infUsername}:${this.infPassword}`)}`,
